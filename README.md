...
## Overview
Non–small cell lung cancer (NSCLC) is the most common type of lung cancer. However, no two cancer diagnoses are exactly alike; this is because different tumors are composed of tumors with significant molecular heterogeneity resulting from differences in intrinsic oncogenic signaling pathways [1]. In addition, patients may have different clinical information that impacts prognosis and treatment options. Better enabling precision medicine, anticipating patient preferences, detecting disease, and improving care quality for NSCLC patients are of utmost importance in the oncology and healthcare and life sciences communities. 

In this solution, we demonstrate how to build a scalable machine learning pipeline for the [Non-Small Cell Lung Cancer (NSCLC) Radiogenomics dataset](https://wiki.cancerimagingarchive.net/display/Public/NSCLC+Radiogenomics), which consists of RNA sequencing data, clinical data (reflective of EHR data), and medical images [2]. Using multiple types of data to create a machine model is refered to as *multi-modal* ML. We predict survival outcome of patients diagnosed with non-small cell lung cancer.
...
For genomic data, we keep 21 genes from 10 highly coexpressed gene clusters (metagenes) that were identified, validated in publicly available gene-expression cohorts, and correlated with prognosis [3]. Please see `1_preprocess_genomic_data.ipynb` for detail.

For clinical data, we perform one hot encoding to convert categorical attributes to numerical attributes, remove columns such as dates that do not provide information, and remove rows with missing values. Please see `2_preprocess_clinical_data.ipynb` for detail.

For medical imaging data, we create patient-level 3-dimensional radiomic features that explain the size, shape and visual attributes of the tumors observed in the CT scans. For each patient study, the following steps is performed. Please see `3_preprocess_imaging_data.ipynb` for detail.
...
![sol-arch-diagram](https://sagemaker-solutions-prod-us-east-2.s3-us-east-2.amazonaws.com/sagemaker-lung-cancer-survival-prediction/1.0.0/docs/multimodal-architecture-overall-jumpstart-20220217.png)
...
![imaging-pipeline](https://sagemaker-solutions-prod-us-east-2.s3-us-east-2.amazonaws.com/sagemaker-lung-cancer-survival-prediction/1.0.0/docs/multimodal-architecture-medical-imaging.png)
...
*Caution*: You need to manually delete any other resources that you may have created while running the notebook, such as Amazon S3 buckets to the solution's default bucket, Amazon SageMaker Feature Store, Amazon SageMaker Experiments, and Amazon SageMaker endpoint.
